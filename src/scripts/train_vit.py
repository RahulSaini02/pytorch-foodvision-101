...
from src.modeling.models import create_vit_model
...
MODEL_NAME = f"vit_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pth"
RESULTS_PATH = Path("reports/vit_results.json")

# Create model
model, transform = create_vit_model(num_classes=len(CLASS_NAMES))
